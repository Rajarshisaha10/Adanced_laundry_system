...
def login_attempt():
    ...
    if success:
        ...
        # Do not persist VTOP credentials; keep only a short-lived session cookie.
        resp.set_cookie('session_id', s_id, httponly=True, samesite='Lax')
...
def get_credentials_api():
    ...
    try:
        ...
        logger.debug("Fetching credentials for %s...", authorized_id)
        ...
        logger.debug("Error fetching credentials for session %s: %s", session_id, e)
...
def get_profile_api():
    ...
    try:
        ...
        logger.debug("Error fetching profile for session %s: %s", session_id, e)
        return jsonify({'status': 'error', 'message': str(e)}), 500
