...
def get_dashboard_summary():
    ...
    pending_collection = LaundryBatch.query.filter_by(status='collected').count()
...
def register_vtop_student():
    ...
    try:
        ...
        reg_no = (data.get('educational') or {}).get('reg_no') or (data.get('personal') or {}).get('reg_no') or authorized_id
        hostel_block = (data.get('hostel') or {}).get('block')
        room_number = (data.get('hostel') or {}).get('room') or ''
        phone_number = (data.get('personal') or {}).get('mobile') or ''
        floor = _parse_room_number(hostel_block) or _parse_room_number(room_number) or 1
...
def claim_student_token(id):
    ...
    # Keep token namespace clean to avoid conflicts while resolving batch/student tokens.
    existing_batch = LaundryBatch.query.filter_by(token=token).first()
    if existing_batch:
        # Allow same student to reuse their old completed token by reopening that batch.
        if existing_batch.student_id == student.id and existing_batch.status == 'pickedUp':
            existing_batch.status = 'collected'
            existing_batch.collected_at = datetime.now()
            existing_batch.washed_at = None
            existing_batch.picked_up_at = None
            _create_notification(student.id, existing_batch.id, "collected")
            _upsert_daily_detail(student.id, existing_batch.id, "collected", student_room)
            db.session.commit()
            return jsonify(student_schema.dump(student)), 200
        return jsonify({"error": "Token already in use for a laundry batch"}), 400
...
def update_student(id):
    ...
    if 'floor' in data: student.floor = data['floor']
    if 'roomNumber' in data: student.room_number = data['roomNumber']
...
def create_booking():
    ...
    active = LaundryBatch.query.filter(
        LaundryBatch.student_id == student.id,
        LaundryBatch.status.in_(["pending", "collected", "washing", "washed"])
    ).first()
...
def create_batch_by_own_token():
    ...
    existing_batch = LaundryBatch.query.filter_by(token=token).first()
    if existing_batch and existing_batch.student_id != student.id:
        return jsonify({"error": "Token number already in use"}), 400
...
def generate_student_token_from_ocr():
    ...
    if existing_active_batch and _has_generated_token(existing_active_batch):
        return jsonify({"error": "A token already exists for your current laundry. You can generate a new one only after pickup."}), 409
...
def get_current_token_generation_state():
    ...
    return jsonify({
        ...
        "canGenerate": not (active_batch and _has_generated_token(active_batch)),
